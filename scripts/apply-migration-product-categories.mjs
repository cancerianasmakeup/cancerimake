...
if (!password) {
  console.error("Faltó DB_PASS. Ejecutá así:\n  DB_PASS=tu_password node scripts/apply-migration-product-categories.mjs");
  process.exit(1);
}

const SQL_FILE = path.resolve(
  process.cwd(),
  "supabase/migrations/20260516000001_product_categories_multi.sql"
);
...
try {
  ...
  const r = await client.query(`
    SELECT
      (SELECT COUNT(*) FROM product_categories) AS pc_count,
      (SELECT COUNT(*) FROM product_categories WHERE is_primary) AS pc_primary,
      (SELECT COUNT(*) FROM products WHERE category_id IS NOT NULL) AS products_with_cat
  `);
  console.log("Estado post-migración:", r.rows[0]);
} catch (e) {
