          ...
          <tbody>
            {customers?.map((c: any) => (
              <tr key={c.id} className="border-t border-rose-pastel hover:bg-rose-whisper/50">
                <td className="p-4 font-semibold text-ink-primary">{c.full_name || "—"}</td>
            ...
            {(!customers || customers.length === 0) && (
              <tr><td colSpan={4} className="text-center py-16 text-ink-soft">Todavía no hay clientas registradas.</td></tr>
            )}
          </tbody>
        </table>
      </div>
